...
public class VINT {
    ...
    public VINT(long binary, byte length, long value) {
        if (binary == 0L) {
            ...
            bs.set(length * ParserUtils.BIT_IN_BYTE - length);
    ...
    public static VINT fromBinary(long binary) {
        ...
        byte length = 1;
        if (bs.length() > 3 * ParserUtils.BIT_IN_BYTE) {
            mask = MASK_BYTE_4;
            length = 4;
        } else if (bs.length() > 2 * ParserUtils.BIT_IN_BYTE) {
            mask = MASK_BYTE_3;
            length = 3;
        } else if (bs.length() > 1 * ParserUtils.BIT_IN_BYTE) {
            mask = MASK_BYTE_2;
            length = 2;
        }
    ...
    public static VINT fromValue(long value) {
        BitSet bs = BitSet.valueOf(new long[] { value });
        byte length = (byte) (1 + bs.length() / ParserUtils.BIT_IN_BYTE);
        if (bs.length() == length * ParserUtils.BIT_IN_BYTE) {
            length++;
        }
        bs.set(length * ParserUtils.BIT_IN_BYTE - length);
