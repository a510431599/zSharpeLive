...
public class CachingFileKeyFrameMetaCache extends FileKeyFrameMetaCache {
    ...
    private void freeCachingMetadata() {
        ...
        for (Map.Entry<String, IKeyFrameDataAnalyzer.KeyFrameMeta> cacheEntry : inMemoryMetaCache.entrySet()) {
            if (randomIndex == 0) {
        ...
        try {
            ...
            if (!inMemoryMetaCache.containsKey(canonicalPath)) {
                ...
                try {
                    ...
                    IKeyFrameDataAnalyzer.KeyFrameMeta keyFrameMeta = super.loadKeyFrameMeta(file);
