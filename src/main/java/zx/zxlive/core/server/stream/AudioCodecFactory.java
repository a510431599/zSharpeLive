...
public class AudioCodecFactory {
    ...
    public static IAudioStreamCodec getAudioCodec(IoBuffer data) {
        ...
        try {
            ...
            switch (codecId) {
                case 10: //aac 
                    result = (IAudioStreamCodec) Class.forName("zx.zxlive.common.codec.AACAudio").getDeclaredConstructor().newInstance();
                    break;
                case 11:
                    result = (IAudioStreamCodec) Class.forName("zx.zxlive.common.codec.SpeexAudio").getDeclaredConstructor().newInstance();
                    break;
                case 13:
                    result = (IAudioStreamCodec) Class.forName("zx.zxlive.common.codec.OpusAudio").getDeclaredConstructor().newInstance();
                    break;
                case 2:
                case 14:
                    result = (IAudioStreamCodec) Class.forName("zx.zxlive.common.codec.MP3Audio").getDeclaredConstructor().newInstance();
                    break;
            }
            data.rewind();
        } catch (Exception ex) {
