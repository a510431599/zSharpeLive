...
package zx.zxlive.core.server.net.rtmp.codec;
...
import zx.zxlive.core.server.net.protocol.ProtocolException;
import zx.zxlive.core.server.net.protocol.RTMPDecodeState;
import zx.zxlive.core.server.net.rtmp.message.ChunkHeader;
import zx.zxlive.core.server.net.rtmp.message.Header;
import zx.zxlive.core.server.net.rtmp.message.Packet;
import zx.zxlive.core.server.net.rtmp.message.SharedObjectTypeMapping;
import zx.zxlive.core.server.net.rtmp.status.Status;
import zx.zxlive.core.server.net.rtmp.status.StatusCodes;
import zx.zxlive.core.server.service.PendingCall;
import zx.zxlive.core.server.stream.StreamService;
import zx.zxlive.common.io.amf.AMF;
import zx.zxlive.common.io.amf.Output;
import zx.zxlive.common.io.object.DataTypes;
import zx.zxlive.common.io.object.Deserializer;
import zx.zxlive.common.io.object.Input;
import zx.zxlive.common.io.object.StreamAction;
import zx.zxlive.common.io.amf3.AMF3;
import zx.zxlive.core.server.api.Red5;
import zx.zxlive.core.server.net.rtmp.RTMPConnection;
import zx.zxlive.core.server.net.rtmp.RTMPUtils;
import zx.zxlive.core.server.net.rtmp.event.Abort;
import zx.zxlive.core.server.net.rtmp.event.Aggregate;
import zx.zxlive.core.server.net.rtmp.event.AudioData;
import zx.zxlive.core.server.net.rtmp.event.BytesRead;
import zx.zxlive.core.server.net.rtmp.event.ChunkSize;
import zx.zxlive.core.server.net.rtmp.event.ClientBW;
import zx.zxlive.core.server.net.rtmp.event.FlexMessage;
...
import zx.zxlive.core.server.net.rtmp.event.SWFResponse;
import zx.zxlive.core.server.net.rtmp.event.ServerBW;
...
import zx.zxlive.core.server.so.FlexSharedObjectMessage;
import zx.zxlive.core.server.so.ISharedObjectEvent;
import zx.zxlive.core.server.so.ISharedObjectMessage;
...
public class RTMPProtocolDecoder implements Constants, IEventDecoder {
    ...
    public ISharedObjectMessage decodeFlexSharedObject(IoBuffer in) {
        ...
        Input input;
        if (encoding == 0) {
            input = new zx.zxlive.common.io.amf.Input(in);
        } else if (encoding == 3) {
            input = new zx.zxlive.common.io.amf3.Input(in);
        } else {
    ...
    public ISharedObjectMessage decodeSharedObject(IoBuffer in) {
        final Input input = new zx.zxlive.common.io.amf.Input(in);
        String name = input.getString();
        // Read version of SO to modify
        int version = in.getInt();
        // Read persistence informations
    ...
    protected void doDecodeSharedObject(SharedObjectMessage so, IoBuffer in, Input input) {
        // Parse request body
        Input amf3Input = new zx.zxlive.common.io.amf3.Input(in);
            ...
            if (type == ISharedObjectEvent.Type.CLIENT_STATUS) {
                ...
                if (length > 0) {
                    ...
                    if (length > key.length() + 2) {
                        ...
                        Input propertyInput;
                        if (objType == AMF.TYPE_AMF3_OBJECT && !(input instanceof zx.zxlive.common.io.amf3.Input)) {
    ...
    private Invoke decodeAction(IConnection.Encoding encoding, IoBuffer in, Header header) {
        ...
        if (encoding == IConnection.Encoding.AMF3 && tmp == AMF.TYPE_AMF3_OBJECT) {
            input = new zx.zxlive.common.io.amf3.Input(in);
            ((zx.zxlive.common.io.amf3.Input) input).enforceAMF3();
        } else {
            input = new zx.zxlive.common.io.amf.Input(in);
        }
        // get the action
    ...
    public Notify decodeStreamData(IoBuffer in) {
        ...
        Input input = new zx.zxlive.common.io.amf.Input(in);
        ...
        if (dataType == DataTypes.CORE_STRING) {
            ...
            if ("@setDataFrame".equals(action)) {
                ...
                /*
                 * IoBuffer buf = IoBuffer.allocate(64); buf.setAutoExpand(true); Output out = null; if (encoding == Encoding.AMF3) { out = new zx.zxlive.common.io.amf3.Output(buf); } else { out = new
    ...
    public FlexMessage decodeFlexMessage(IoBuffer in) {
        ...
        zx.zxlive.common.io.amf3.Input.RefStorage refStorage = new zx.zxlive.common.io.amf3.Input.RefStorage();
        Input input = new zx.zxlive.common.io.amf.Input(in);
        String action = Deserializer.deserialize(input, String.class);
            ...
            while (in.hasRemaining()) {
                ...
                switch (objectEncodingType) {
                    ...
                    case AMF3.TYPE_VECTOR_OBJECT:
                        // The next parameter is encoded using AMF3
                        input = new zx.zxlive.common.io.amf3.Input(in, refStorage);
