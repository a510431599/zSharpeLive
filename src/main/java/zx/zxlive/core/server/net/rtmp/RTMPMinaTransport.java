 ...
 */
@Component
public class RTMPMinaTransport implements RTMPMinaTransportBean {
    ...
    @Value("#{'${rtmp.addresses}'.split(',')}")
    protected Set<String> addresses = new HashSet<>();

    @Autowired
    @Qualifier("rtmpMinaIoHandler")
    ...
    public void start() throws Exception {
        ...
        if (enableDefaultAcceptor) {
            ...
            SimpleIoProcessorPool<NioSession> pool = new SimpleIoProcessorPool<>(NioProcessor.class, maxProcessorPoolSize);
...
//            }
            ...
            if (enableMinaMonitor) {
                ...
                stats = new IoServiceStatistics(acceptor);
                //poll every second
                ...
                try {
                    ...
                    mbs.registerMBean(new StandardMBean(this, RTMPMinaTransportBean.class, true), serviceManagerObjectName);
                } catch (Exception e) {
